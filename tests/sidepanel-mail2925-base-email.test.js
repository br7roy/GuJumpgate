...
const inputHostedCheckoutVerificationPopupDelaySeconds = { value: '4' };
const inputHostedCheckoutVerificationUrl = { value: '' };
const inputHostedCheckoutPhone = { value: '' };
const inputHostedCheckoutSmsPool = { value: '' };
const inputHotmailAliasEnabled = { checked: false };
const inputOutlookAliasMaxPerAccount = { value: '5' };
...
function normalizeOutlookAliasMaxPerAccount(value) { const parsed = Number.parseInt(String(value ?? '').trim(), 10); return Number.isFinite(parsed) ? Math.min(50, Math.max(1, parsed)) : 5; }
function normalizeVerificationResendCount(value, fallback) { return Number(value) || fallback; }
function normalizeSupportedMailProvider(value = '') { return String(value || '').trim().toLowerCase() === 'cloudflare-temp-email' ? 'cloudflare-temp-email' : 'hotmail-api'; }
