...
test('Plus checkout billing searches checkout tabs inside the locked automation window', async () => {
  ...
  const { checkoutTab, executor } = createExecutorHarness({
    ...
    queryTabsInAutomationWindow: async (queryInfo) => {
      ...
      if (Array.isArray(queryInfo?.url) && queryInfo.url.includes('https://chatgpt.com/checkout/*')) {
        return [checkoutTab];
      }
      return [];
    },
  ...
  assert.deepEqual(queries, [
    ...
    { url: ['https://chatgpt.com/checkout/*', 'https://pay.openai.com/c/pay/*', 'https://checkout.stripe.com/c/pay/*'] },
  ]);
});

test('Plus checkout billing accepts pay.openai hosted checkout tabs from step 6 redirect chain', async () => {
  const { checkoutTab, events, executor } = createExecutorHarness({
    frames: [{ frameId: 0, url: 'https://pay.openai.com/c/pay/cs_test' }],
    stateByFrame: {
      0: {
  ...
  checkoutTab.url = 'https://pay.openai.com/c/pay/cs_test';

  await executor.executePlusCheckoutBilling({
    plusCheckoutTabId: checkoutTab.id,
    plusCheckoutUrl: 'https://pay.openai.com/c/pay/cs_test',
  });

  assert.deepEqual(events.ensuredTabs[0], { source: 'plus-checkout', tabId: checkoutTab.id });
  assert.equal(events.completed[0].step, 'plus-checkout-billing');
  assert.equal(events.messages.some((entry) => entry.message.type === 'PLUS_CHECKOUT_SELECT_PAYPAL'), true);
});
...
test('Plus checkout billing uses PayPal US autocomplete strategy when explicitly enabled', async () => {
  const fetchRequests = [];
  const { events, executor } = createExecutorHarness({
    frames: [
      { frameId: 0, url: 'https://chatgpt.com/checkout/openai_ie/cs_test' },
    ...
    getAddressSeedForCountry: (countryValue) => {
      if (/us/i.test(String(countryValue || ''))) return createUsAddressSeed();
  ...
  await executor.executePlusCheckoutBilling({
    ...
    paypalCheckoutUsAutocompleteModeEnabled: true,
  });
  ...
  assert.equal(ensureAddressMessage.frameId, 8);
  assert.equal(combinedFillMessage, undefined);
  assert.equal(fillQueryMessage.message.payload.addressSeed.countryCode, 'US');
  assert.match(fillQueryMessage.message.payload.addressSeed.query, /^\d{4}$/);
  assert.equal(fillQueryMessage.message.payload.addressSeed.randomSuggestion, true);
  assert.equal(fillQueryMessage.message.payload.addressSeed.forceCountrySelectionBeforeAutocomplete, true);
  assert.equal(fillQueryMessage.message.payload.addressSeed.autoCheckAgreement, true);
  assert.equal(fetchRequests.length, 0);
});

test('Plus checkout billing uses randomuser US address seed by default on pay.openai checkout', async () => {
  const fetchRequests = [];
  const { events, executor } = createExecutorHarness({
    frames: [
      { frameId: 0, url: 'https://pay.openai.com/c/pay/cs_test' },
      { frameId: 7, url: 'https://js.stripe.com/v3/elements-inner-payment.html' },
      { frameId: 8, url: 'https://js.stripe.com/v3/elements-inner-address.html' },
    ],
    stateByFrame: {
    ...
    getAddressSeedForCountry: () => createUsAddressSeed(),
    fetchImpl: async (url, init) => {
      fetchRequests.push({ url, init });
      return {
        ok: true,
        ...
        json: async () => ({
          results: [{
            location: {
              street: { number: 1034, name: 'Burke Street' },
              city: 'Boston',
              state: 'Massachusetts',
              postcode: '02210',
            },
  ...
  assert.equal(combinedFillMessage.message.payload.addressSeed.skipAutocomplete, true);
  assert.equal(combinedFillMessage.message.payload.addressSeed.fallback.address1, '1034 Burke Street');
  assert.equal(combinedFillMessage.message.payload.addressSeed.fallback.city, 'Boston');
  assert.equal(combinedFillMessage.message.payload.addressSeed.fallback.region, 'Massachusetts');
  assert.equal(combinedFillMessage.message.payload.addressSeed.fallback.postalCode, '02210');
  assert.equal(fetchRequests[0].url, 'https://randomuser.me/api/?nat=us&inc=location&noinfo');
});

test('Plus checkout billing still uses meiguodizhi country paths for GoPay localized countries without local seeds', async () => {
  const fetchRequests = [];
  const { events, executor } = createExecutorHarness({
    frames: [
      { frameId: 0, url: 'https://chatgpt.com/checkout/openai_llc/cs_test' },
...
test('GPC billing polls queue task, submits WhatsApp OTP then PIN, and waits until completed', async () => {
  ...
  const { events, executor } = createExecutorHarness({
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_123') {
        pollCount += 1;
        if (pollCount === 1) {
          return {
            ok: true,
  ...
  const run = executor.executePlusCheckoutBilling({
    ...
    gopayHelperApiUrl: 'https://your-gpc-helper-domain.example/api/gp/tasks/task_old/otp',
  ...
  assert.equal(fetchCalls[0].url, 'https://your-gpc-helper-domain.example/api/gp/tasks/task_123');
...
test('GPC billing auto mode only polls until completed without OTP or PIN submission', async () => {
  ...
  const { events, executor } = createExecutorHarness({
    ...
    fetchImpl: async (url, options = {}) => {
      fetchCalls.push({ url, options });
      if (url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_auto') {
        pollCount += 1;
        if (pollCount === 1) {
          return {
            ok: true,
...
test('GPC billing logs checkout order stage in Chinese', async () => {
  ...
  const { events, executor } = createExecutorHarness({
    ...
    stateByFrame: {},
    fetchImpl: async (url) => {
      if (url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_stage') {
        pollCount += 1;
        if (pollCount === 1) {
          return {
            ok: true,
...
test('GPC billing fails repeated checkout stage as stale so auto-run can recreate task', async () => {
  ...
  const { events, executor } = createExecutorHarness({
    ...
    fetchImpl: async (url, options = {}) => {
      fetchCalls.push({ url, options });
      if (url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_stale') {
        return {
          ok: true,
          status: 200,
          json: async () => createGpcTaskResponse({
...
test('GPC billing fails unchanged visible created status even when hidden ids change', async () => {
  ...
  const { events, executor } = createExecutorHarness({
    ...
    fetchImpl: async (url, options = {}) => {
      fetchCalls.push({ url, options });
      if (url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_created') {
        queryCount += 1;
        return {
          ok: true,
          status: 200,
...
test('GPC billing reads SMS OTP from local helper for sms_otp_wait', async () => {
  ...
  const { events, executor } = createExecutorHarness({
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_sms') {
        pollCount += 1;
        if (pollCount === 1) {
          return {
            ok: true,
...
test('GPC billing can read WhatsApp OTP from local helper when enabled', async () => {
  ...
  const { events, executor } = createExecutorHarness({
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_wa') {
        pollCount += 1;
        if (pollCount === 1) {
          return {
            ok: true,
...
test('GPC billing helper mode does not open OTP dialog when helper has no code and task times out', async () => {
  ...
  const { events, executor } = createExecutorHarness({
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_timeout') {
        const queryCount = fetchCalls.filter((call) => call.url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_timeout').length;
        return {
          ok: true,
          status: 200,
...
test('GPC billing helper mode requests newer OTP after invalid OTP error', async () => {
  ...
  const { events, executor } = createExecutorHarness({
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_retry') {
        taskPollCount += 1;
        if (taskPollCount === 1) {
...
test('GPC billing manual OTP wrong input opens next dialog only after previous one closes', async () => {
  ...
  const { events, executor } = createExecutorHarness({
    ...
    fetchImpl: async (url, options = {}) => {
      fetchCalls.push({ url, options });
      if (url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_manual_retry') {
        pollCount += 1;
        if (pollCount === 1) {
...
test('GPC billing manual OTP cancel stops task and ends current round', async () => {
  ...
  const { events, executor } = createExecutorHarness({
    ...
    fetchImpl: async (url, options = {}) => {
      fetchCalls.push({ url, options });
      if (url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_cancel') {
...
test('GPC billing PIN failure ends task without retrying PIN', async () => {
  ...
  const { executor } = createExecutorHarness({
    ...
    fetchImpl: async (url, options = {}) => {
      fetchCalls.push({ url, options });
      if (url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_pin_failed') {
        pollCount += 1;
        if (pollCount === 1) {
...
for (const terminalStatus of ['failed', 'expired', 'discarded']) {
  test(`GPC billing throws readable error for terminal ${terminalStatus} task`, async () => {
    ...
    const { executor } = createExecutorHarness({
      ...
      fetchImpl: async (url, options = {}) => {
        fetchCalls.push({ url, options });
        if (url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_bad') {
          return {
            ok: true,
            status: 200,
            json: async () => createGpcTaskResponse({
...
test('GPC billing stops task best-effort when flow is interrupted before terminal state', async () => {
  ...
  const { executor } = createExecutorHarness({
    ...
    fetchImpl: async (url, options = {}) => {
      fetchCalls.push({ url, options });
      if (url === 'https://your-gpc-helper-domain.example/api/gp/tasks/task_stop') {
        return {
          ok: false,
          status: 500,
